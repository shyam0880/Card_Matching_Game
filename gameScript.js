var em = ["image1.jpg", "image2.jpg", "image3.jpg", "image4.jpg", "image5.jpg", "image6.jpg", "image7.jpg", "image8.jpg"];
// Shuffle the array
...
function start(r, l) {
    ...
    for (var i = 1; i <= r; i++) {
        $("table").append("<tr>");
        for (var j = 1; j <= l; j++) {
        $("table").append(`<td id='${n}' onclick="change(${n})"><div class='inner'><div class='front'></div><div class='back'><img src='images/${items[n - 1]}' alt='Image'><div class='hide-text'>${items[n - 1]}</div></div></div></td>`);
        n++;
        }
        $("table").append("</tr>");
    }
...
function change(x) {
    ...
    //Flip
      ...
      if (rem==0) {
            ...
            setTimeout(function() {
                $("#ol").html(`<center><div id="iol"><h2>Congrats!</h2><p style="font-size:23px;padding:10px;">You completed the ${mode} mode in ${moves} moves. It took you ${time}.</p><p style="font-size:18px">Comment Your Score!<br/>Play Again ?</p><button onclick="start(3, 4)">3 x 4</button> <button onclick="start(4, 4)" style="w">4 x 4</button><button onclick="start(4, 5)">4 x 5</button><button onclick="start(4, 6)">4 x 6</button></div></center>`);
                $("#ol").fadeIn(750);
            }, 1500);
      }
    }
